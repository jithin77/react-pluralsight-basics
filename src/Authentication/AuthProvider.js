    ...
    async getAccessToken(scopes) {
      ...
      try {
        ...
        if (process.env.NODE_ENV === 'production') {
          ...
          silentResult = await this.publicClientApplication.ssoSilent(accessRequest);
        } else if (process.env.NODE_ENV === 'development') {
          silentResult = await this.publicClientApplication.acquireTokenSilent(accessRequest);
          console.log('silentRequest', silentResult);
        }
        return silentResult.accessToken;
      } catch (err) {
        if (this.isInteractionRequired(err)) {
          var interactiveResult = await this.publicClientApplication.acquireTokenPopup(accessRequest);
